...
# NEW: Compute how many distinct characters use this component
...
# UPDATED: Now accepts usage count for the selected component
def generate_clean_card_html(c, usage_count=None):
    ...
    # NEW: Show how many characters use this component
...
def render_splash():
    ...
    for c in component_map.keys():
        ...
        if sc is None or sc < 3:
            continue
        comps.append(c)
    comps_sorted = sorted(comps, key=lambda c: -component_usage_count(c))
    MAX_DEMOS = 20
    demos = []
    seen = set()
    for c in preferred:
...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.stroke_view_active:
            ...
            if st.session_state.preview_comp:
                ...
                st.markdown(
                    f"<div class='preview-count-line'>{count} characters contain <span class='char'>{preview_char}</span></div>",
                    unsafe_allow_html=True,
                )
        else:
            st.button("← Back to list", on_click=back, use_container_width=True)
            ...
            if selected_char:
                ...
                st.markdown(
                    f"<div class='preview-count-line'>{count_filtered} characters contain <span class='char'>{selected_char}</span></div>",
                    unsafe_allow_html=True,
                )
    ...
    if st.session_state.show_inputs:
        ...
        # Sort by usage frequency (descending), then strokes
        sorted_comps = sorted(
            filtered,
            key=lambda c: (-component_usage_count(c), get_stroke_count(c) or 999, c)
        )
        if not sorted_comps:
            ...
            with p2:
                ...
                st.markdown(
                    ...
                    <div style='text-align:center; padding:10px 0; color:#555;'>
                        ...
                        <div style='font-size:0.85em; color:#e74c3c;'>Sorted by frequency of use as component</div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
        ...
        # UPDATED: Sort by each character's own usage frequency (descending), then stroke count
        chars = sorted(
            chars,
            key=lambda x: (-component_usage_count(x), get_stroke_count(x) or 999, x)
        )
        if st.session_state.script_variant == "Simplified":
        ...
        for c in chars:
            ...
            with col_btn:
                # Vertical spacing to align buttons with the large character
            ...
            with col_char:
                # THIS IS THE CLICKABLE LARGE CHARACTER
                if st.button(
                    c,
                    key=f"explore_char_{c}",
                    use_container_width=True,
                    help=f"Explore all characters that contain {c} as a component",
                ):
                    enter_component(c)
                    st.rerun()
            ...
            with col_details:
                ...
                st.markdown(
                    generate_clean_card_html(c, usage_count=usage_count),
                    unsafe_allow_html=True
                )

                # Compounds / phrases display (unchanged)
                if compounds.get(c):
                    ...
                    for word in sorted_compounds:
                        ...
                        if entry:
                            items_html.append(
                                ...
                                f"<span class='cp-pinyin'>{display_pinyin}</span>"
                    ...
                    st.markdown(
                        ...
                        <div style='padding:15px; background:#f1f8e9; border-radius:8px; 
                             margin-top:10px; border:1px solid #dcedc8; max-height:400px; overflow-y:auto;'>
