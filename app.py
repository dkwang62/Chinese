# app.py
# Radix — Component-first Chinese character explorer (Ultimate Complete Version)
...
# --- Session-state filter caching ---
if "rad_groups" not in st.session_state:
    st.session_state.rad_groups = stats_cache["rad_groups"]
if "idc_counts" not in st.session_state:
    st.session_state.idc_counts = stats_cache["idc_counts"]
...
def apply_dynamic_css():
    ...
    /* TRADITIONAL VARIANT - Orange/Yellow */
    .meta-tag-trad {
        background: #fff8e1;
        color: #856404;
        padding: 6px 14px;
    ...
    .comp-grid .stButton > button:hover {
        ...
        box-shadow: 0 8px 20px rgba(192, 57, 43, 0.18) !important;
    }
    ...
    .char-btn-wrap .stButton > button:hover {
        ...
        box-shadow: 0 8px 25px rgba(66, 165, 245, 0.25) !important;
    }
    ...
    .pen-btn-wrap .stButton > button:hover {
        ...
        box-shadow: 0 6px 16px rgba(100, 181, 246, 0.25) !important;
    }
...
def generate_clean_card_html(c, usage_count=None):
    ...
    # Variant tags with special classes
    if cc_t2s:
        simplified = cc_t2s.convert(c)
        if simplified != c and simplified in component_map:
            meta_items.append(f"<span class='meta-tag meta-tag-simp'>Simp. → {simplified}</span>")
    if cc_s2t:
        traditional = cc_s2t.convert(c)
        if traditional != c and traditional in component_map:
            meta_items.append(f"<span class='meta-tag meta-tag-trad'>Trad. → {traditional}</span>")
                    ...
                    for i, r in enumerate(rads):
                        with cols[i % 5]:
                            if st.button(r, key=f"rad_{r}", type="primary" if st.session_state.radical == r else "secondary"):
                                st.session_state.radical = r if st.session_state.radical != r else "none"
                                st.session_state.page = 1
                                st.rerun()
                ...
                for i, idc in enumerate(idc_keys):
                    with cols[i % 5]:
                        if st.button(idc, key=f"idc_{idc}", type="primary" if st.session_state.component_idc == idc else "secondary"):
                            st.session_state.component_idc = idc if st.session_state.component_idc != idc else "none"
                            st.session_state.page = 1
                            st.rerun()
