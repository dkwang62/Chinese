...
def render_grid_view():
    ...
    with tab_search:
        st.markdown("<div style='background: #f0f8ff; padding: 20px; border-radius: 10px; margin-bottom: 25px; border: 1px solid #d0e8ff;'>", unsafe_allow_html=True)
        st.markdown("**Smart Search**")
        st.markdown("Find characters by fuzzy pinyin (e.g., 'ma', 'shu3') or English meaning (e.g., 'horse', 'water').")
        
        search_query = st.text_input("Enter Pinyin or Meaning", key="grid_tab_search_input", placeholder="e.g. ni, ma, water, sun")
        st.markdown("</div>", unsafe_allow_html=True)
        ...
        if search_query:
            ...
            if len(query) < 2:
                ...
                results = []
                query_normalized = normalize_pinyin(query)

                for char, info in component_map.items():
                    meta = info.get("meta", {})
                    
                    # 1. Match Pinyin (Fuzzy)
                    pinyin_raw = meta.get("pinyin", "")
                    if pinyin_raw:
                        pinyin_norm = normalize_pinyin(pinyin_raw)
                        # Exact match or contains if query is long enough
                        if query_normalized == pinyin_norm or (len(query_normalized) > 2 and query_normalized in pinyin_norm):
                            results.append(char)
                            continue
                    
                    # 2. Match Definition (English)
                    definition = meta.get("definition", "")
                    if definition and query in definition.lower():
                        results.append(char)
                        continue
                ...
                if not results:
                    ...
                    # Render Grid of Results
                    st.markdown("<div class='comp-grid'>", unsafe_allow_html=True)
                    
                    # Limit results to 100 to prevent lag
                    display_results = results[:100]
                    cols = st.columns(GRID_COLUMNS)
                    for i, ch in enumerate(display_results):
                        with cols[i % GRID_COLUMNS]:
                            st.button(ch, key=f"search_res_{ch}_{i}", type="primary" if state.get_preview_component() == ch else "secondary", on_click=tile_click, args=(ch,), use_container_width=True)
                    st.markdown("</div>", unsafe_allow_html=True)
